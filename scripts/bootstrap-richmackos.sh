...
echo "[1/9] Updating Debian..."
apt update

echo "[2/9] Installing RichmackOS base packages..."
grep -vE '^[[:space:]]*(#|$)' config/packages.txt | xargs apt install -y

echo "[3/9] Creating RichmackOS filesystem..."
mkdir -p \
    /opt/richmack/bin \
    /opt/richmack/apps \
...
echo "[4/9] Installing RichmackOS identity..."
cat > /etc/richmack-release <<'RELEASE'
NAME="RichmackOS"
VERSION="0.3.0"
ID=richmackos
ID_LIKE=debian
PRETTY_NAME="RichmackOS 0.3.0 (Debian 13)"
ARCHITECTURE="ARM64"
BUILD="Development"
RELEASE

echo "[5/9] Installing RichmackOS artwork..."
install -m 0644 assets/logo.ans /opt/richmack/assets/logo.ans

echo "[6/9] Installing RichmackOS commands..."
